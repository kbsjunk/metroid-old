...
//specified by easting (x-coordinate) with key 'x' and northing (y-coordinate) with key 'y'.
...
//values along with any others.
    function simplifyPolyline($pointList, $epsilon)
    {
    // Find the point with the maximum distance
        ...
        {
            $d = $this->perpendicularDistance($pointList[$i]['x'], $pointList[$i]['y'],
    ...
    // If max distance is greater than epsilon, recursively simplify
        ...
        {
        // Recursive call
            $recResults1 = $this->simplifyPolyline(array_slice($pointList, 0, $index + 1), $epsilon);
            $recResults2 = $this->simplifyPolyline(array_slice($pointList, $index, $totalPoints - $index), $epsilon);
